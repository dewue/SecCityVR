...
public class PlayerSpawner : MonoBehaviour, INetworkRunnerCallbacks
{
    ...
    public Dictionary<PlayerRef, NetworkObject> spawnedUsers = new Dictionary<PlayerRef, NetworkObject>();
    ...
    public void OnPlayerJoined(NetworkRunner runner, PlayerRef player)
    {
        if (player == runner.LocalPlayer && playerPrefab != null)
        {
            ...
            string name = (player.PlayerId > 1 ? "Developer" : "Auditor");
            networkPlayerObject.transform.Find("Player/Head/NameCanvas/Name").GetComponent<TextMeshProUGUI>().text = name;
            // Keep track of the player avatars so we can remove it when they disconnect
            spawnedUsers.Add(player, networkPlayerObject);
            Debug.LogWarning("### Spawned # " + spawnedUsers.Count);
        }
    }
